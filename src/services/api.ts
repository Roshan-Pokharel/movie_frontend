...
export const api = {
  search:    (q: string, type = '', page = 1) =>
    request(`${BASE}/search?q=${encodeURIComponent(q)}&type=${type}&page=${page}`),
  trending:  () => request(`${BASE}/trending`),
  stats:     () => request(`${BASE}/stats`),
  details:   (imdbId: string) => request(`${BASE}/${imdbId}`),
  library:   (params: Record<string, string | number | boolean> = {}) =>
    request(`${BASE}/library?${qs(params)}`),
  favorites: () => request(`${BASE}/favorites`),
  bookmarks: () => request(`${BASE}/bookmarks`),
  watched:   (params: Record<string, string | number | boolean> = {}) =>
    request(`${BASE}/watched?${qs(params)}`),
  ratings:   () => request(`${BASE}/ratings`),
  save:      (body: object) => request(BASE, { method: 'POST', body: JSON.stringify(body) }),
